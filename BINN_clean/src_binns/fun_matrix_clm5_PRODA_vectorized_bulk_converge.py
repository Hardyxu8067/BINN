...
def fun_PRODA_bulk_simu(tensor_para, tensor_frocing_steady_state):
	...
	bulk_K_all = (torch.ones((profile_num, 1))*np.nan).to(device)
	bulk_K_soc = (torch.ones((profile_num, 1))*np.nan).to(device)
	bulk_K_litter = (torch.ones((profile_num, 1))*np.nan).to(device)
	...
	for iprofile in range(0, profile_num):
		...
		if torch.isnan(torch.sum(profile_para)) == False and \
			...
			# print("bulk_K_all_profile", bulk_K_all_profile.shape)
			# print("bulk_K_soc_profile", bulk_K_soc_profile.shape)
			# print("bulk_K_litter_profile", bulk_K_litter_profile.shape)
			...
			bulk_K_all[iprofile, :] = bulk_K_all_profile
			bulk_K_soc[iprofile, :] = bulk_K_soc_profile
			bulk_K_litter[iprofile, :] = bulk_K_litter_profile
		...
		# end if 
	#end for iprofile
	return carbon_input, cpool_steady_state, cpools_layer, soc_layer, total_res_time, total_res_time_base, res_time_base_pools, t_scaler, bulk_A, w_scaler, bulk_K_all, bulk_K_soc, bulk_K_litter, bulk_V, bulk_xi, bulk_I, litter_fraction
...
def fun_matrix_clm5(para, frocing_steady_state):
	...
	# print("cpools_total", cpools_total.shape)
	# calculate bulk_K
	bulk_K_all = torch.sum(decom_cpools * (cpools_total / torch.sum(cpools_total))).to(device)
	bulk_K_soc = torch.sum(decom_cpools[4:7] * (cpools_total[4:7] / torch.sum(cpools_total[4:7]))).to(device)
	bulk_K_litter = torch.sum(decom_cpools[1:4] * (cpools_total[1:4] / torch.sum(cpools_total[1:4]))).to(device)
